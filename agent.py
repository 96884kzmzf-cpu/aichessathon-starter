"""AI Chessathon V3c: V3b safety baseline with low-overhead bucketed move ordering.

Derived directly from V3b. RFP and hard futility remain disabled. Main/root move
ordering consumes one already-generated legal move list, partitions it once into
cheap priority buckets, and sorts only within buckets that benefit from ranking.
Qsearch and all search selectivity are unchanged from V3b.
Standard-library + python-chess runtime only.
"""
import math
import time
...
ENGINE_VERSION = "v3c_bucket_order"
MATE_SCORE = 100_000
INFINITY = 1_000_000
MAX_DEPTH = 64
...
RFP_MAX_DEPTH = 2
RFP_MARGIN_PER_PLY = 160
FUTILITY_MAX_DEPTH = 1
FUTILITY_MARGIN_PER_PLY = 140
FUTILITY_START_MOVE = 3
USE_POSITIONAL_UPDATES = True
# V2a: leaf evaluation is intentionally lean; depth is worth more than expensive
# mobility/attack-zone detail in Python.
USE_FAST_EVAL = True
# Full SEE inside main-search move ordering is too expensive in Python.
# Qsearch still uses SEE selectively for suspicious deep captures.
USE_SEE_ORDERING = False
# Conservative qsearch pruning. These affect only non-checking, non-promotion
# captures when a stand-pat score is available; evasions are never pruned.
USE_QSEARCH_PRUNING = True
QSEARCH_DELTA_MARGIN = 140
QSEARCH_SEE_PRUNE_THRESHOLD = -80
QUIET_THREAT_PLIES = 2
MAX_PLY = 256  # Emergency abort; never static-evaluate a checked leaf.
...
_EVAL_CACHE = [None] * EVAL_CACHE_SIZE
_EVAL_HITS = 0
_EVAL_MISSES = 0
...
# Best quiet reply to the opponent's previous move; cheap continuation ordering.
_COUNTERMOVES: list[list[chess.Move | None]] = [[None for _ in range(4096)] for _ in range(2)]
...
def _material_pst(pieces: int, piece_type: int, color: chess.Color) -> tuple[int, int, int]:
    """Exact reusable material/PST subtotal for one piece type and side.

    A move usually changes only two of twelve configurations. Pawn-dependent
    rook/outpost terms and king shelter remain outside this cache.
    """
    count = pieces.bit_count()
    mg = MG_VALUE.get(piece_type, 0) * count
    eg = EG_VALUE.get(piece_type, 0) * count
    if piece_type == chess.BISHOP and count >= 2:
        mg += 28
        eg += 42
    table_mg, table_eg = MG_PST[piece_type], EG_PST[piece_type]
    flip = 56 if color else 0
    for sq in chess.scan_forward(pieces):
        index = sq ^ flip
        mg += table_mg[index]
        eg += table_eg[index]
...
def _evaluate_white_fast(board: chess.Board) -> int:
    """Lean tapered evaluator for V2a.

    Keep terms that are cheap, stable, and mostly bitboard/cache based. Expensive
    per-piece attack generation, pin maps, king attack zones, and mobility are
    deliberately omitted so alpha-beta can buy another completed ply.
    """
    white = board.occupied_co[chess.WHITE]
    black = board.occupied_co[chess.BLACK]
    pawns = [board.pawns & black, board.pawns & white]
    ...
    piece_masks = (0, board.pawns, board.knights, board.bishops,
                   board.rooks, board.queens, board.kings)
    for color in (chess.WHITE, chess.BLACK):
        sign = 1 if color else -1
        own = board.occupied_co[color]
        for piece_type in chess.PIECE_TYPES:
            pieces = piece_masks[piece_type] & own
            subtotal_mg, subtotal_eg, count = _material_pst(pieces, piece_type, color)
            mg += sign * subtotal_mg
            eg += sign * subtotal_eg
            material[color] += EG_VALUE.get(piece_type, 0) * count
            phase += PHASE_WEIGHT.get(piece_type, 0) * count
            if piece_type in PHASE_WEIGHT:
                nonpawn_eg[color] += EG_VALUE[piece_type] * count
        ...
        for sq in chess.scan_forward(board.rooks & own):
            file_mask = chess.BB_FILES[sq & 7]
            if not pawns[color] & file_mask:
                is_open = not pawns[not color] & file_mask
                mg += sign * (18 if is_open else 10)
                eg += sign * (12 if is_open else 7)
            if _relative_rank(sq, color) == 6:
                mg += sign * 20
                eg += sign * 30
        outposts = (board.knights & own & pawn_attacks[color]
                    & ~pawn_attacks[not color])
        for sq in chess.scan_forward(outposts):
            rank = _relative_rank(sq, color)
            if 3 <= rank <= 5:
                mg += sign * (18 + 4 * (rank - 3))
                eg += sign * 10

    # Cheap king shelter only. The V2 attack-zone machinery called attacks_mask()
    # repeatedly at every leaf and was disproportionately expensive in Python.
    for color in (chess.WHITE, chess.BLACK):
        king = kings[color]
        if king is None:
            continue
        sign = 1 if color else -1
        enemy = not color
        scale = min(100, max(20, nonpawn_eg[enemy] * 100 // 3200))
        if not board.queens & board.occupied_co[enemy]:
            scale = scale * 45 // 100
        mg += sign * (_shelter(pawns[color], pawns[enemy], king, color) * scale // 100)

    # Preserve the cheap king-vs-passer and mop-up guidance from V2.
    for color in (chess.WHITE, chess.BLACK):
        king = kings[color]
        enemy_king = kings[not color]
...
def _evaluate(board: chess.Board, key: PositionKey | None = None) -> int:
    global _EVAL_HITS, _EVAL_MISSES
    if key is None:
        key = _position_key(board)
    index = hash(key) & (EVAL_CACHE_SIZE - 1)
    entry = _EVAL_CACHE[index]
    if entry is not None and entry[0] == key:
        _EVAL_HITS += 1
        return entry[1]
    _EVAL_MISSES += 1
    score = _evaluate_white_fast(board) if USE_FAST_EVAL else _evaluate_white(board)
    if not board.turn:
        score = -score
    _EVAL_CACHE[index] = (key, score)
    return score
...
def _see(board: chess.Board, move: chess.Move) -> int:
    """Legal least-valuable-attacker exchange estimate.
    ...
    qsearch therefore uses it only as a conservative deep-capture filter.
    """
...
def _move_order_score(
    ...
    if board.is_capture(move):
        ...
        # A high-value attacker taking an undefended victim should not sit
        # behind every ordinary quiet. Remove the source to expose x-ray
        # defenders; EP remains with the conservative baseline ordering.
        if not board.is_en_passant(move) and not board.attackers_mask(
            not board.turn, move.to_square,
            board.occupied & ~chess.BB_SQUARES[move.from_square],
        ):
            return 7_000_000 + capture
    ...
    if board.move_stack:
        previous = board.peek()
        counter = _COUNTERMOVES[int(board.turn)][previous.from_square * 64 + previous.to_square]
        if move == counter:
            return 5_800_000 + _history_score(board.turn, move)
...
def _bucket_ordered_moves(
    ...
    """Order an already-generated legal list with low Python overhead.

    This preserves V3b's broad score bands but avoids its global expensive key
    function. In particular, killer/countermove/history state is fetched once
    per node rather than once per quiet move. No legal moves are regenerated,
    no sets are built, and no extra legality probes are performed.
    """
    turn = board.turn
    history = _HISTORY[int(turn)]
    ...
    if board.move_stack:
        previous = board.peek()
        counter = _COUNTERMOVES[int(turn)][previous.from_square * 64 + previous.to_square]

    preferred_move: chess.Move | None = None
    promotions: list[chess.Move] = []
    good_captures: list[tuple[int, chess.Move]] = []
    killer0_move: chess.Move | None = None
    killer1_move: chess.Move | None = None
    counter_move: chess.Move | None = None
    ...
    for move in moves:
        ...
        # V3b ranks every promotion above ordinary captures, including capture
        # promotions. Preserve that policy exactly.
        if move.promotion is not None:
        ...
        if board.is_capture(move):
            ...
            victim = chess.PAWN if en_passant else board.piece_type_at(move.to_square)
            if attacker is None or victim is None:
                capture_score = 0
                good = False
            else:
                capture_score = 10 * ORDER_VALUE[victim] - ORDER_VALUE[attacker]
                good = ORDER_VALUE[victim] >= ORDER_VALUE[attacker]
                if not good and not en_passant:
                    # Same V3b cheap undefended-capture test. Removing the
                    # source square exposes x-ray defenders on the destination.
                    good = not board.attackers_mask(
                        not turn,
                        move.to_square,
                        board.occupied & ~chess.BB_SQUARES[move.from_square],
                    )
                if not good and USE_SEE_ORDERING and use_see:
                    good = _see(board, move) >= 0

            (good_captures if good else bad_captures).append((capture_score, move))
            continue
    ...
    # Sorting is limited to bands where V3b had a meaningful secondary score.
    # Python sort is stable, so equal-scored moves retain generator order.
    promotions.sort(key=lambda move: ORDER_VALUE.get(move.promotion, 0), reverse=True)
    good_captures.sort(key=lambda item: item[0], reverse=True)
    quiets.sort(
        key=lambda move: history[move.from_square][move.to_square],
        reverse=True,
    )
    ...
    if preferred_move is not None:
        ordered.append(preferred_move)
    ordered.extend(promotions)
    ordered.extend(move for _, move in good_captures)
    if killer0_move is not None:
        ordered.append(killer0_move)
    if killer1_move is not None:
        ordered.append(killer1_move)
    if counter_move is not None:
        ordered.append(counter_move)
    ordered.extend(quiets)
    ordered.extend(move for _, move in bad_captures)
    return ordered


def _update_history(turn: chess.Color, move: chess.Move, bonus: int) -> None:
    row = _HISTORY[int(turn)][move.from_square]
    value = row[move.to_square]
    row[move.to_square] = value + bonus - value * abs(bonus) // 8192
...
def _record_quiet_cutoff(
    ...
    bonus = min(512, 12 * depth * depth)
    _update_history(turn, move, bonus)
    if board.move_stack:
        previous = board.peek()
        _COUNTERMOVES[int(turn)][previous.from_square * 64 + previous.to_square] = move
...
def _quiescence(
    ...
    """Tight capture quiescence.

    V2 spent most of its nodes below depth zero. V2a does the minimum work
    needed to avoid evaluating obviously unstable capture sequences: stand pat
    first, all legal evasions in check, then captures/promotions only. Expensive
    SEE is reserved for deep, clearly suspicious captures.
    """
    global _QTHREAT_NODES, _QDELTA_PRUNES, _QSEE_PRUNES
    _tick(ply, qnode=True)

    in_check = board.is_check()
    # Checkmate/stalemate take precedence over draw claims and static bounds.
    legal_iter = board.generate_legal_moves()
    first_legal = next(legal_iter, None)
    ...
    if board.is_insufficient_material():
        return 0
    if ply >= MAX_PLY - 1:
        raise SearchTimeout  # Discard iteration; keep last completed root result.
    alpha = max(alpha, -MATE_SCORE + ply)
    beta = min(beta, MATE_SCORE - ply - 1)
    if alpha >= beta:
        return alpha
    if qply >= MAX_QUIESCENCE_PLY and not in_check:
        return _evaluate(board, key)
    if in_check:
        # In check there is no stand pat: every legal evasion must remain.
        moves = [first_legal]
        moves.extend(legal_iter)
        if not moves:
            return -MATE_SCORE + ply
        best = -INFINITY
        stand_pat: int | None = None
    else:
        # Crucial speed change: do not materialize every legal quiet move before
        # stand-pat. Most qnodes end here or after a small capture set.
        stand_pat = _evaluate(board, key)
        if stand_pat >= beta:
            return stand_pat
        best = stand_pat
        if stand_pat > alpha:
        ...
        # Include non-capturing promotions. Queen promotion is overwhelmingly
        # dominant, but retain every legal promotion for correctness.
        promotion_rank = chess.BB_RANK_7 if board.turn else chess.BB_RANK_2
        moves.extend(
            board.generate_legal_moves(
                from_mask=board.pawns & promotion_rank,
    ...
    # MVV/LVA + promotion ordering only. Full SEE in the sorting key was a poor
    # trade in Python; selective SEE below is much cheaper.
    ordered = _ordered_moves(board, moves, ply=ply, use_see=False)
    for move in ordered:
        is_capture = board.is_capture(move)

        if (
            USE_QSEARCH_PRUNING
            and stand_pat is not None
            and is_capture
            ...
            victim_value = ORDER_VALUE.get(victim or 0, 0)

            # Cheap delta test first. We intentionally do not call gives_check()
            # on every capture: that legality/attack calculation cost was large.
            # Checks that win enough material survive this generous margin.
            if stand_pat + victim_value + QSEARCH_DELTA_MARGIN <= alpha:
                if not board.gives_check(move):
                    _QDELTA_PRUNES += 1
                    continue

            attacker = board.piece_type_at(move.from_square)
            # SEE is expensive. Use it only deeper in qsearch and only when a
            # much more valuable attacker takes a cheaper victim.
            if (
                qply >= 2
                and attacker is not None
                and victim is not None
                and ORDER_VALUE[attacker] >= ORDER_VALUE[victim] + 200
                and not board.gives_check(move)
                and _see(board, move) < QSEARCH_SEE_PRUNE_THRESHOLD
            ):
                _QSEE_PRUNES += 1
                continue
...
def _negamax(
    ...
    global _RFP_CUTOFFS, _FUTILITY_PRUNES
    ...
    # V3b keeps the V3a/V2e pruning code available for diagnostics/ablation, but
    # both hard static-pruning switches are disabled in this release.  Avoid even
    # computing a static evaluation here when neither rule can use it.
    side_non_pawn = (board.knights | board.bishops | board.rooks | board.queens) & board.occupied_co[board.turn]
    static_eval: int | None = None
    static_pruning_ok = (
        not pv_node
        and not in_check
        and bool(side_non_pawn)
        and alpha > -MATE_THRESHOLD
        and beta < MATE_THRESHOLD
    )
    if (
        (USE_REVERSE_FUTILITY or USE_FUTILITY_PRUNING)
        and static_pruning_ok
        and depth <= max(RFP_MAX_DEPTH, FUTILITY_MAX_DEPTH)
    ):
        static_eval = _evaluate(board, key)

    if (
        USE_REVERSE_FUTILITY
        and static_eval is not None
        and depth <= RFP_MAX_DEPTH
        and static_eval - RFP_MARGIN_PER_PLY * depth >= beta
    ):
        _RFP_CUTOFFS += 1
        return static_eval
    ...
    moves = [first_legal]
    moves.extend(legal_iter)
    ordered = _bucket_ordered_moves(board, moves, preferred, ply)
    best = -INFINITY
    best_move = ordered[0]
    failed_quiets = []
    ...
    for move_index, move in enumerate(ordered):
        ...
        # Hard futility is disabled in V3b. This block remains behind the feature flag
        # so future ablations can re-enable it without changing search semantics.
        if (
            futility_pruning
            and move_index >= FUTILITY_START_MOVE
            and quiet
            and move != preferred
        ...
        if alpha >= beta:
            if quiet:
                ...
                penalty = -min(256, 6 * depth * depth)
                for failed in failed_quiets:
                    _update_history(turn, failed, penalty)
            break
        if quiet:
            failed_quiets.append(move)
...
def _search_root(
    ...
    global _PVS_RESEARCHES, _ROOT_MOVES
    _ROOT_MOVES = 0
    ...
    original_alpha = alpha
    original_beta = beta
    ordered = _bucket_ordered_moves(board, moves, preferred, 0)
    best_move = ordered[0]
    best_score = -INFINITY
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    global _RFP_CUTOFFS, _FUTILITY_PRUNES
    global _QTHREAT_NODES, _QDELTA_PRUNES, _QSEE_PRUNES
    global _TT_PROBES, _PV_REUSES, _ASPIRATION_RETRIES
    global _PV_LINE, _EVAL_HITS, _EVAL_MISSES
    ...
    best_score = 0
    completed_depth = 0
    completed_nodes = completed_qnodes = completed_root_moves = 0
    ...
    for depth in range(completed_depth + 1, MAX_DEPTH + 1):
        ...
        completed_nodes, completed_qnodes = _NODES, _QNODES
        completed_root_moves = _ROOT_MOVES
    ...
    print(
        ...
        f"version={ENGINE_VERSION} completed_nodes={completed_nodes} "
        f"completed_qnodes={completed_qnodes} completed_root_moves={completed_root_moves} "
        f"root_legal_moves={len(legal_moves)} "
        ...
        f"lmr_researches={_LMR_RESEARCHES} rfp_cutoffs={_RFP_CUTOFFS} "
        f"futility_prunes={_FUTILITY_PRUNES} qthreat_nodes={_QTHREAT_NODES} "
        f"qdelta_prunes={_QDELTA_PRUNES} qsee_prunes={_QSEE_PRUNES} "
        f"tt_probes={_TT_PROBES} pv_reuses={_PV_REUSES} "
        f"aspiration_retries={_ASPIRATION_RETRIES} order_scheme=bucket_once move={best_move.uci()} "
        f"eval_cache_hits={_EVAL_HITS} "
        f"eval_cache_misses={_EVAL_MISSES} "
