...
def _evaluate_white_fast(board: chess.Board) -> int:
    ...
    for color in (chess.WHITE, chess.BLACK):
        ...
        for piece_type in chess.PIECE_TYPES:
            ...
            table_mg = MG_PST[piece_type]
            table_eg = EG_PST[piece_type]
            for sq in chess.scan_forward(pieces):
                index = _pst_index(sq, color)
                mg += sign * table_mg[index]
                eg += sign * table_eg[index]
                ...
                if piece_type == chess.ROOK:
                    ...
                    if 3 <= rank <= 5 and pawn_attacks[color] & bit and not pawn_attacks[not color] & bit:
                        mg += sign * (18 + 4 * (rank - 3))
                        eg += sign * 10
