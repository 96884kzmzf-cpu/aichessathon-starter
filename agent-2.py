...
def _board_with_history(fen: str) -> chess.Board:
    """Recover the opponent's last move and retain the actual game's move stack."""
    ...
    # The process normally serves one game. If a local caller skips moves or
    # starts another game, discard stale history instead of inventing moves.
    return incoming


def _remember_move(board: chess.Board, move: chess.Move) -> str:
    """Keep the position after our real move, never a speculative search branch."""
    global _GAME_BOARD
...
def _piece_square_bonus(piece_type: int, square: int, color: chess.Color, endgame: bool) -> int:
...
def _move_order_score(board: chess.Board, move: chess.Move, preferred: chess.Move | None) -> int:
...
def _quiescence(board: chess.Board, alpha: int, beta: int, ply: int, qply: int = 0) -> int:
    ...
    # The referee ends the game on the third occurrence, not the second.
    # The stack includes real game moves plus this speculative search line.
    if board.is_repetition(3):
        return 0

    # Counterchecking evasions can keep q-search going without captures.
    # Abort this iteration at the safety limit; never stand pat in check.
...
def _negamax(board: chess.Board, depth: int, alpha: int, beta: int, ply: int) -> int:
    """Negamax search with alpha-beta pruning and quiescent leaves."""
    if depth == 0:
